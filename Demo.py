    ...
    def query(self):
        ...
        try:
            try:
                ...
                self.trains = self.res.json()['data']['result']
            Trains_demo(txt_show_ticket, self.trains, self.ticket_option).show_train_data()
        except:
            title = '提示'
            message = '出现问题，请重新点击按钮！'
...
class Buy_Tickets(object):
    ...
    def order_transfer(self):
        self.headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/71.0.3578.98 Safari/537.36'}
        self.res = requests.get(self.url_template.format('A', self.dtime, self.starts, self.ends), headers=self.headers,
                                verify=False)
        try:
            try:
                self.trains = self.res.json()['data']['result']
            except:
                self.res = requests.get(self.url_template.format('Z', self.dtime, self.starts, self.ends),
